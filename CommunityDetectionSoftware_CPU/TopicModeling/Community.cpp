...
timestamp_t measure_start_svd_m, measure_stop_svd_m;       // Timing for svd1
double time_svd_m;
...
int write_pi(char *filename, SparseMatrix<double> mat)
{
  ...
  for(long i = 0; i < spmat.rows(); i++)
    for(long j = 0; j < spmat.cols(); j++)
      if(spmat.coeff(i, j) != 0)
	f << i << "\t" << j << "\t" << spmat.coeff(i, j) << endl;
  f.close();
  */
...
{
//  furongprintVector(x, len, "aaaaaaaaaaaaaaaaaaaaaa");
...
/*
 ...
 {
 double B=exp(log(gamma(z))+log(gamma(w))-log(gamma(z+w)));
 return B;
 }
 */
...
{
  //SparseMatrix<double> random_mat(B.cols(),k);
    cout<<"-----------begining of random_embeding_mat---------------"<<endl;
    // cout<<"B.nonZeros(): "<<B.nonZeros()<< endl;
    // cout <<"B is :\n" << (MatrixXd)B<<endl;
    // D
    // SparseMatrix<double> D;
    // D.resize(B.rows(), B.cols());D.setIdentity();
    // srand((int)clock());
    // for (int i = 0; i < D.rows(); i++)
    // {
        
    //     int r = rand()%5 ;
    //     double p_rand = (double)r/(double)5.0;
    //     if (p_rand<0.5) {
    //         D.coeffRef(i, i)= -1;
    //     }
    ...
    // Phi
    // (1) nonzero index_set
//     vector<int> nonzero_set;
//  srand((int)clock());
//     for(int i = 0; i < B.cols(); i++)
//     {
//         SparseVector<double> curr_col = B.col(i);
// //	cout <<"curr_col: \n"<< (VectorXd)curr_col<<endl;
//         SparseVector<double> curr_row = B.block(i,0,1,B.cols());
// //        cout <<"curr_row: \n"<< (VectorXd)curr_row<<endl;
//         if (curr_col.cwiseAbs().sum()!=0 && curr_row.cwiseAbs().sum()!=0) {
//             nonzero_set.push_back(i);
// //	    cout <<"nonZero Index: "<< i<<endl;
//        }
//     }
    // (2)basis row vec phi (B.cols()\times k)
    srand(clock());
...
//    cout<<"phi:\n"<<(MatrixXd)phi<<endl;
//    random_mat = phi;
//    random_mat.makeCompressed();
//    random_mat.prune(EPS);
//cout << "random_mat:\n"<<random_mat<<endl;
//    return random_mat;
}
//////////////////////////////////////////////////////////////////////////////////////
pair< SparseMatrix<double>, SparseVector<double> > SVD_symNystrom_sparse(SparseMatrix<double> A,int k)
{
    ...
    Q.prune(EPS);
    // W
    MatrixXd W = (MatrixXd)random_mat.transpose() * A * random_mat;
    MatrixXd W_sqrt = sqrt_matrix(W);
    SparseMatrix<double> W_sqrt_sparse = W_sqrt.sparseView();
    // getting column span u
    SparseMatrix<double> u = Q * R_inv_sparse.transpose() * W_sqrt_sparse.transpose();
...
pair<SparseMatrix<double>,SparseMatrix<double> > pinv_asymNystrom_sparse(SparseMatrix<double> X)
{
    ...
    result = pinv_symNystrom_sparse(X.transpose()*X);
...
//    pair<SparseMatrix<double>, SparseMatrix<double> > pairB = pinv_asymNystrom_sparse(Z_B_denominator);
    ...
    cout << "C_inv_pairC:rows():"<< C_inv_pairB.rows()<< ";cols():"<<C_inv_pairB.cols()<<endl;
    cout << "W_pairC: rows():"<< W_pairB.rows()<< "; cols():"<< W_pairB.cols()<<endl;
...
if(NA+NB+NC<200000)
 ...
 SparseMatrix<double> Z_C = Z_C_numerator * Z_C_tmp4;
    ...
    cout << "K:"<<KHID<<endl;
    cout << "NA:"<< NA<<",NB:"<< NB <<",NC:"<< NC;
    cout << "C_inv_pairC:rows():"<< C_inv_pairC.rows()<< ";cols():"<<C_inv_pairC.cols()<<endl;
    cout << "W_pairC: rows():"<< W_pairC.rows()<< "; cols():"<< W_pairC.cols()<<endl;
 cout << "-----------Z_C_numerator:nonZeros()" << Z_C_numerator.nonZeros()<< "-------------"<<endl;
    ...
    cout <<"-------------calculating M2_tmp1--------------------"<<endl;
    SparseMatrix<double> M2_tmp1 = Z_B_numerator.transpose() * random_mat;
    cout << "---------dimension: "<< M2_tmp1.rows() << " , "<<M2_tmp1.cols()<<endl;
    cout <<"-------------calculating M2_tmp2--------------------"<<endl;
    SparseMatrix<double> M2_tmp2 = C_inv_pairB * M2_tmp1;
    cout << "---------dimension: "<< M2_tmp2.rows() << " , "<<M2_tmp2.cols()<<endl;
    cout <<"-------------calculating M2_tmp3--------------------"<<endl;
    SparseMatrix<double> M2_tmp3 = W_pairB.transpose() * M2_tmp2;
    cout << "---------dimension: "<< M2_tmp3.rows() << " , "<<M2_tmp3.cols()<<endl;
    cout <<"-------------calculating M2_tmp4--------------------"<<endl;
    SparseMatrix<double> M2_tmp4 = C_inv_pairB.transpose() * M2_tmp3;
    cout << "---------dimension: "<< M2_tmp4.rows() << " , "<<M2_tmp4.cols()<<endl;
    cout <<"-------------calculating M2_tmp5--------------------"<<endl;
    ...
    cout << "---------dimension: "<< M2_tmp5.rows() << " , "<<M2_tmp5.cols()<<endl;
    cout <<"-------------calculating M2_tmp6--------------------"<<endl;
    SparseMatrix<double> M2_tmp6 = Z_C_denominator  *M2_tmp5;
    cout << "---------dimension: "<< M2_tmp6.rows() << " , "<<M2_tmp6.cols()<<endl;
    cout <<"-------------calculating M2_tmp7--------------------"<<endl;
    ...
    cout << "---------dimension: "<< M2_tmp7.rows() << " , "<<M2_tmp7.cols()<<endl;
    cout <<"-------------calculating M2_tmp8--------------------"<<endl;
    SparseMatrix<double> M2_tmp8 = C_inv_pairC  *M2_tmp7;
    cout << "---------dimension: "<< M2_tmp8.rows() << " , "<<M2_tmp8.cols()<<endl;
    cout <<"-----------calculating M2_tmp9------------------"<<endl;
    SparseMatrix<double> M2_tmp9 = W_pairC * M2_tmp8;
    cout << "---------dimension: "<< M2_tmp9.rows() << " , "<<M2_tmp9.cols()<<endl;
     cout <<"-----------calculating M2_tmp10------------------"<<endl;
    SparseMatrix<double> M2_tmp10 = C_inv_pairC.transpose() * M2_tmp9;
    cout << "---------dimension: "<< M2_tmp10.rows() << " , "<<M2_tmp10.cols()<<endl;
    cout <<"-------------calculating M2--------------------"<<endl;
    SparseMatrix<double> M2 = Z_C_numerator * M2_tmp10;
    cout << "---------dimension: "<< M2.rows() << " , "<<M2.cols()<<endl;
    
    M2.makeCompressed();
    //    M2 = inv_nx * M2;//All the pairs are normalized, and mu_a, mu_b, mu_c are also normalized. No need to normalize again. 
...
///    cout <<"M2:\n" << M2 << endl;
    ...
    cout << "-------mu_a_sparse_proj dimension: "<< mu_a_sparse_proj.size()<<endl;
    SparseMatrix<double> square_mu_a_sparse_proj = mu_a_sparse* mu_a_sparse_proj.transpose();
  cout << "-------square_mu_a_sparse_proj dimension: "<< square_mu_a_sparse_proj.size()<<endl;
    square_mu_a_sparse_proj = square_mu_a_sparse_proj - tmp_square_proj;
    cout <<"---------square_mu_a_sparse_proj dimension: "<< square_mu_a_sparse_proj.rows() << " , "<< square_mu_a_sparse_proj.cols()<<endl;
    ...
    cout <<"-----------C.first row:\n" << C.row(0)<< endl;
    /*
     MatrixXd C_inv_dense = pinv_matrix((MatrixXd)C);
     SparseMatrix<double> C_inv = C_inv_dense.sparseView();
    */
    SparseMatrix<double> C_inv = pinv_matrix(C);//!!!!this step doesn't work
    // SparseMatrix<double> C_inv =  pinv_asymNystrom_sparse(C);
    cout << "----------C_inv.dimension:" << C_inv.rows()<<","<< C_inv.cols()<<endl;
    cout << "-----------C_inv.nonZeros():"<<C_inv.nonZeros()<<"-------"<<endl;
    // W_nystrom
    MatrixXd W_nystrom = (MatrixXd)random_mat.transpose() * C;
    MatrixXd W_nystrom_sqrt = sqrt_matrix(W_nystrom);
    cout << "-----------W_nystrom_sqrt:n" << W_nystrom_sqrt<< endl;
    ...
    W_nystrom_sqrt_sparse.makeCompressed();W_nystrom_sqrt_sparse.prune(EPS);
     cout << "----------W_nystrom_sqrt_sparse.dimension:" << W_nystrom_sqrt_sparse.rows()<<","<< C_inv.cols()<<endl;
    // getting the witening matrix W
    W.resize(Gx_a.cols(),KHID);
    cout <<"-------C_inv.nonZeros():"<< C_inv.nonZeros()<<endl;
    cout <<"-------W_nystrom_sqrt_sparse.nonZeros()"<<W_nystrom_sqrt_sparse.nonZeros()<<endl;
...
void second_whiten_topic(SparseMatrix<double> Gx_a, SparseMatrix<double> &W, VectorXd &mu_a)
{
  double nx = (double)Gx_a.rows();
    ...
    double inv_nx = 1/((double)nx);
    // compute M2_alpha0 = (1/nx) sum (Gxa' * Gxa) - alpha0/(alpha0+1)*(uxa* uxa'- diag(uxa uxa'))
    Gx_a.prune(EPS);
    ...
    cout << "--------------generating random_mat for M2---------------"<<endl;
    SparseMatrix<double> random_mat = random_embedding_mat((long) Gx_a.cols(),KHID);
    SparseMatrix<double> M2_tmp = Gx_a * random_mat;
    SparseMatrix<double> M2_tmp2 = Gx_a.transpose() * M2_tmp;
    SparseMatrix<double> M2 = inv_nx * M2_tmp2;
    M2.makeCompressed();
    cout << "----------M2.nonZeros():"<< M2.nonZeros() << endl;
    cout << "----------end of caluclating M2, start M2_alpha0-----"<<endl;
...
void tensorDecom_alpha0(SparseMatrix<double> D_a_mat, VectorXd D_a_mu, SparseMatrix<double> D_b_mat, VectorXd D_b_mu, SparseMatrix<double> D_c_mat, VectorXd D_c_mu, VectorXd &lambda, MatrixXd & phi_new)
{
    ...
    while (true)
    {
      // if (LEARNRATE< (double)1/(double)sqrt(iteration))
      // 	{
      // 	  beta = LEARNRATE;
      // 	}
      // else
      // 	{
      // 	  beta = (double)1/(double)sqrt(iteration);
      // 	}
      //      cout << ",beta:" << beta << endl;
...
void tensorDecom_alpha0_topic(SparseMatrix<double> D_a_mat, VectorXd D_a_mu, VectorXd &lambda, MatrixXd & phi_new)
{
	...
	if(iteration%20 == 0)
	  //	  cout << phi_new << endl;        
        phi_old=phi_new;
        //Data_a_g = Data_a_G(:,i);Data_b_g = Data_b_G(:,i);Data_c_g = Data_c_G(:,i);
        phi_new =Diff_Loss(D_a_g,D_a_g,D_a_g,D_a_mu,D_a_mu,D_a_mu,phi_old,beta);
///////////////////////////////////////////////
        if (iteration < MINITER)
        {}
        else
...
MatrixXd Diff_Loss(VectorXd Data_a_g, VectorXd Data_b_g,VectorXd Data_c_g,VectorXd Data_a_mu,VectorXd Data_b_mu,VectorXd Data_c_mu, Eigen::MatrixXd phi,double beta)
{
    MatrixXd New_Phi;
...
VectorXd The_second_term(VectorXd Data_a_g,VectorXd Data_b_g,VectorXd Data_c_g,VectorXd Data_a_mu,VectorXd Data_b_mu,VectorXd Data_c_mu,VectorXd phi)
{
  ...
  double para2 =(double)-alpha0/((double)alpha0+2);
...
Eigen::SparseMatrix<double> read_G_sparse(char *file_name, char *G_name,int N1, int N2) // input: file name, adjacent matrix name, NA/NB/NC, output: sparse matrix
{
    ...
    long trip_len = 0.01 * N1 * N2;
    triplets_sparse.reserve(trip_len);
    //
...
# ifdef EXPECTED
	...
	triplets_sparse.push_back(Triplet<double>((int)(r_idx-1),(int)(c_idx-1),(int)val));
...
# ifdef BINARY
	...
	triplets_sparse.push_back(Triplet<double>((int)(r_idx-1),(int)(c_idx-1),(int)1));
# endif
    }
    fclose(file_ptr);
    cout << "success get the triplets!" << endl;
    G_mat.setFromTriplets(triplets_sparse.begin(),triplets_sparse.end());
    cout << "success get the G_mat!" << endl;
    G_mat.prune(EPS);
    return G_mat;
}
...
Eigen::MatrixXd read_G(char *file_name, char *G_name, int N) // input file name, matrix name, pointer to matrix buffer, (NA or NB or NC)
{
    Eigen::MatrixXd G_mat;
    G_mat = Eigen::MatrixXd::Zero(NX,N);
    double r_idx, c_idx; // row and column indices - matlab style
    printf("reading %s\n", G_name); fflush(stdout);
    cout << file_name << endl;
...
# ifdef EXPECTED
        ...
        G_mat((int) (c_idx-1),(int) (r_idx-1))=1;
# endif
# ifdef BINARY
...
{
    Eigen::MatrixXd G_mat;
    G_mat = Eigen::MatrixXd::Zero(NX, VOCA_SIZE); // number of samples \times number of states (d=2 for binary)
    vector<Eigen::MatrixXd> vec_G_mat;
    double r_idx, c_idx; // row and column indices - matlab style
    ...
    while(!feof(file_ptr)) // reading G_name
    {
        ...
        {
            vec_G_mat.push_back(G_mat);
            G_mat = Eigen::MatrixXd::Zero(NX, VOCA_SIZE); // reinitialize
            continue;
        }
...
# ifdef EXPECTED
        double val;
        fscanf(file_ptr, "%lf", &val);
        G_mat((int)(r_idx-1), (int)(c_idx-1)) = val; // this is now modified in r and c idx; reads in weighted also;
# endif
# ifdef BINARY
        G_mat((int)(r_idx-1), (int)(c_idx-1)) = 1;
# endif
    }
    fclose(file_ptr);
    vec_G_mat.push_back(G_mat);
...
EigenSparseSVD sparse_svd(Eigen::SparseMatrix<double> eigen_sparse_matrix, int rank) // input arguments are a sparse matrix in csc format in eigen toolkit and the rank parameter which corresponds to the number of singular values required; note that this is in double precision
{
    ...
    /***
     // for debugging purpose in small matrix cases: expand the matrix in dense form and display to check if the initializations were right in the svdlibc sparse format
     DMat svdlibc_dense_matrix = svdNewDMat(eigen_sparse_matrix.rows(), eigen_sparse_matrix.cols()); // allocate dynamic memory for a svdlibc dense matrix
     svdlibc_dense_matrix = svdConvertStoD(svdlibc_sparse_matrix); // svdlibc function to convert from sparse to dense format
     for(i=0; i<eigen_sparse_matrix.rows(); i++) // loop for displaying
     {
     for(int j=0; j<eigen_sparse_matrix.cols(); j++)
     printf("%lf ", svdlibc_dense_matrix->value[i][j]);
     printf("\n");
     }
     svdFreeDMat(svdlibc_dense_matrix); // free the dynamic memory allocated for the svdlibc dense matrix
     ***/
